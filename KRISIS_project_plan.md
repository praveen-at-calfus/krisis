...
| 4 to 7 | v1.2 (final) | Retrieval, intelligence, and production-ready | The final planned version, delivered in stages: (1) **retrieval layer** — past resolved tickets embedded + similarity search, shown as a reference panel alongside the classification (not replacing it); (2) **semantic classification cache** — near-duplicate tickets reuse a prior answer and skip the LLM; (3) **confidence-aware routing** — low-confidence tickets flagged for human review instead of auto-routed; (4) **per-ticket cost & usage tracking** on the dashboard; (5) **incident clustering** — consecutive same-category tickets in a short window raise a dismissible dashboard alarm; (6) **production-ready** — config/secrets hardening, pytest suite, and Alembic migrations. The v1.1 category/priority/team logic is preserved; confidence-aware routing *extends* the output with a confidence signal + human-review gate rather than changing how category/priority/team are decided. (The earlier web UI + login/RBAC direction is dropped; the Streamlit UI is retained. Auto-drafted reply suggestions and the misclassification feedback loop are deferred — see Future improvements.) |

## 8. Future improvements

Designed next steps, intentionally not built in this project:

- **Auto-drafted reply suggestions** — generate a suggested response to the employee alongside the classification, for an agent to review and send.
- **Feedback loop for correcting misclassifications** — let agents correct a wrong classification; feed corrections back to improve the prompt/examples (and the retrieval/cache corpus) over time.
- **pgvector** — replace the in-Python cosine search with a native Postgres vector index for scale.
- **Web UI + user login & RBAC** — a dedicated web front end with authentication and roles (employee vs managing team), replacing the tab-scoped Streamlit split.
- **Docker / CI** — containerize the app + Postgres and add continuous integration.

## 9. Summary

The project is structured so that a fully working, fully compliant system exists by the end of day 3. Everything from day 4 onward adds capability on top of a system that already satisfies every requirement. Any enhancement not finished becomes a documented next step (see Future improvements), not a gap in the deliverable.
