...
| 4 to 7 | v1.2 (final) | Retrieval, intelligence, and production-ready | The final planned version, built in stages: (1) **retrieval layer** — past resolved tickets embedded + similarity search, shown as a reference panel alongside the classification (not replacing it); (2) **confidence-aware routing** — low-confidence tickets flagged for human review instead of auto-routed; (3) **per-ticket cost & usage tracking** on the dashboard; (4) **incident clustering** — multiple similar tickets in a short window flagged as one probable incident; (5) **auto-drafted reply suggestions**; (6) a **feedback loop** for correcting misclassified tickets; (7) **production-ready** — deployment/packaging, config & secrets management, migrations, security hardening, observability/logging, tests. The v1.1 category/priority/team logic is preserved; confidence-aware routing *extends* the output with a confidence signal + human-review gate rather than changing how category/priority/team are decided. (The earlier web UI + login/RBAC direction is dropped; the Streamlit UI is retained.) |
